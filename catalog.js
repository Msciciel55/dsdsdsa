...
// --- GŁÓWNA FUNKCJA OTWIERAJĄCA KATALOG ---
...
async function showItemPreview(item) {
    ...
    // Ustaw flagi wizualne i przygotuj elementy
    ...
    // --- LOGIKA ANIMACJI Z MINIMALNYM CZASEM ---

    // 1. Rozpocznij ładowanie modelu w tle.
    const modelContainer = document.getElementById('item-model-container-preview');
    const modelPromise = initItemViewer(item.modelFile, modelContainer);

    // 2. Stwórz drugą obietnicę (Promise), która rozwiąże się po upływie minimalnego czasu.
    // Czas (w ms) powinien odpowiadać czasowi trwania animacji rysowania kręgu.
    const minTimePromise = new Promise(resolve => setTimeout(resolve, 2000)); // 2 sekundy

    // 3. Rozpocznij animację rysowania kręgu (trwa 2 sekundy).
    gsap.to(pathsToAnimate, { 
        strokeDashoffset: 0, 
        duration: 2.0, 
        ease: 'power1.inOut', 
        stagger: 0.2 
    ...
    // 4. Poczekaj na ZAKOŃCZENIE OBU OPERACJI: załadowania modelu ORAZ upłynięcia minimalnego czasu.
    // Promise.all czeka, aż wszystkie obietnice w tablicy zostaną rozwiązane.
    const [itemModel] = await Promise.all([modelPromise, minTimePromise]);
    
    // 5. Sprawdź, czy model załadował się poprawnie. Jeśli nie, przerwij.
    if (!itemModel) {
        console.error("Animacja przerwana z powodu błędu ładowania modelu.");
        await cleanupPreview().then(() => showItemsForPath(currentPath));
        return;
    }

    // 6. Model załadowany I upłynął minimalny czas. Uruchom drugą, główną fazę animacji.
    animationTimeline = gsap.timeline();
    animationTimeline
        .add(() => svgElement.classList.add('is-glowing'))
        .to(itemMaterials.map(m => m.uniforms.u_revealProgress), {
            value: (i) => itemMaterials[i].userData.revealBounds.max + 0.2,
            duration: 2.0,
            ease: 'power2.out',
            stagger: 0.05
        }, ">0.2")
